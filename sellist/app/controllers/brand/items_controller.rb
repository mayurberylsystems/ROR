class Brand::ItemsController < Brand::MainController
  ...
  def download_zip
    ...
    items.each do |item|
      item_path = export_path + item.item_number
  ...
  def item_update_params
    ip = params.permit(:name, :title, :enabled_for_sale, :price, :commission_in_currency)
    ip[:enabled_for_sale] = false if ip[:enabled_for_sale].blank?
    ip
  end

  def item_params
    params.require(:item).permit(:name, :title, :enabled_for_sale, :price, :commission_in_currency)
  end
